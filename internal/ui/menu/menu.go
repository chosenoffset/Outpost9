...
func NewMainMenu(games []gamescanner.GameEntry, r render.Renderer, input render.InputManager, width, height int) *MainMenu {
	return &MainMenu{
		games:           games,
		selectedGame:    0,
		selectedLibrary: 0,
...
func (m *MainMenu) Update() (selected bool, selection Selection) {
	...
	mousePressed := m.input.IsMouseButtonPressed(render.MouseButtonLeft)
	...
	// Keyboard navigation
	if m.input.IsKeyPressed(render.KeyUp) {
		// TODO: Add debouncing for keyboard input
	}
	if m.input.IsKeyPressed(render.KeyDown) {
		// TODO: Add debouncing for keyboard input
	}
	if m.input.IsKeyPressed(render.KeySpace) {
		// Start selected game
...
func (m *MainMenu) Draw(screen render.Image) {
