...
type Manager struct {
	...
	loader         render.ResourceLoader // Resource loader for loading images
}

// NewManager creates a new atlas manager
func NewManager(loader render.ResourceLoader) *Manager {
	return &Manager{
...
func (m *Manager) DrawTile(screen render.Image, layer, tileName string, x, y float64) error {
...
func (m *Manager) DrawTileWithOptions(screen render.Image, layer, tileName string, opts *render.DrawImageOptions) error {
	atlas, ok := m.GetAtlasByLayer(layer)
	if !ok {
		return fmt.Errorf("no atlas found for layer: %s", layer)
	}
